...
def attach_servo(pin):
    pi.set_mode(pin, pigpio.OUTPUT)

def detach_servo(pin):
    pi.set_mode(pin, pigpio.INPUT)

def set_angle(pin, angle):
    pulse_width = 1000 + (angle * ((2000-1000)/180))
    pi.set_servo_pulsewidth(pin, pulse_width)

def sweep_servo(pin, start_angle, end_angle, duration=1.0):
    """
    Sweeps a servo between start and end angles over the specified duration.

    :param pin: GPIO pin for the servo
    :param start_angle: Starting angle of the servo sweep
    :param end_angle: Ending angle of the servo sweep
    :param duration: Total time in seconds for the sweep
    """
    step_count = abs(end_angle - start_angle)
    if step_count == 0:
        return
    sleep_time = duration / step_count

    # Determine if we're increasing or decreasing angle
...
def sendchat(prompt, transcript):
    completion = openai.ChatCompletion.create(
        model=CHATGPT_MODEL,
        messages=[
            {"role": "system", "content": SYSTEM_ROLE},
            {"role": "assistant", "content": ''.join(transcript)},
...
class Bear:
    def __init__(self, azure_speech_config):
        ...
        # self._arms_motor = Servo(25)

        # self._mouth_motor = kit.motor2
        self.mouth_servo_pin = servo_pins[0]
        self.mouth_stop_event = threading.Event()  # Add this
    ...
    def move_mouth(self, angle):
        set_angle(self.mouth_servo_pin, angle)
    ...
    def move_mouth_threaded(self):
        # while not self.mouth_stop_event.is_set():  # Change the loop condition
        #     self.move_mouth(45)  # open mouth
        #     time.sleep(0.5)
        #     self.move_mouth(0)   # close mouth
        #     time.sleep(0.5)
        while not self.mouth_stop_event.is_set():  # Change the loop condition
            sweep_servo(self.mouth_servo_pin, 0, 45, duration=0.5)  # open mouth smoothly
            sweep_servo(self.mouth_servo_pin, 45, 0, duration=0.5)  # close mouth smoothly
    ...
    def speak(self, text):
        self.mouth_stop_event.clear()  # Clear the event before starting mouth movement
        self._mouth_thread = threading.Thread(target=self.move_mouth_threaded, daemon=True)
        self._mouth_thread.start()
        # self.move_mouth(45) 
        result = self._speech_synthesizer.speak_text_async(text).get()
        # self.move_mouth(0)
        self.mouth_stop_event.set()  # Set the event after speaking to stop mouth movement
        self._mouth_thread.join()
...
def main():
    ...
    transcription = load_memory()

    #transcription = [""]
    bear.speak(
        "Hello! Squeeze my left arm to talk to me!"
    )
            ...
            if listener.speech_waiting():
                ...
                if text:
                    if listener.phrase_complete:
                        ...
                        chat_response = sendchat(text, transcription)
